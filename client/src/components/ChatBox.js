  ...
  return (
    ...
    {/* Mobile Navbar */}
      <nav className="md:hidden flex items-center justify-between bg-green-600 px-4 py-3 mb-2">
        ...
        <div className="text-xl md:text-2xl pt-2 font-extrabold text-gray-100">Inbox</div>
        {selectedUserName && (
          <div className="ml-2 md:ml-4 px-2 pt-2 md:px-4 py-1 md:py-2 rounded-lg bg-green-100 text-green-700 font-bold text-base md:text-lg">
            {selectedUserName}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-2 py-3 pb-2 md:pb-4 scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        {selectedUserId ? (
          <>
            {messages.length > 0 ? (
              messages.map((msg) => (
